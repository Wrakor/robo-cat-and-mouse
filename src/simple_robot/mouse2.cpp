...
int main(int argc, char ** argv) {
	ros::init(argc, argv, "mouse2");
	ros::NodeHandle nh;
	
	// create a publisher object
	cmd_vel_pub_ = nh.advertise<geometry_msgs::Twist>("robot3/cmd_vel", 1000);
	...
	ros::Subscriber subCat1 = nh.subscribe("robot1/odom", 1000, &cat1Pos);
        ros::Subscriber subMouse = nh.subscribe("robot3/odom", 1000, &mousePos);
	...
	while(ros::ok) {
		...
		if (distance <= 1.5 && distance != 0) {
			char cmd[50] = "rosrun stdr_robot robot_handler delete /robot3";			
